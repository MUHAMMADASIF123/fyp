import React from 'react';
import './BsMass_Communication.css';

function BsMass_Communication() {
  return (
    <>
    <div className='container mt-2'>
      <div className='row'>
        <div className='col-md-6  col-sm-12 mb-2 semester  rounded bg-white '>
        <h2 className='rounded'>
      Bachelor of Science in Communication Studies (Hons)
    </h2>
    <hr />
    <h4 className='paragraph rounded'>Course Object</h4>
    <hr />
    <p className='paragraph'>
      The Bachelor of Science in Communication Studies will
      help students explore the theory and practice of
      human communication, with a primary emphasis on
      Communication Media skills and a secondary emphasis
      on oral and written language skills.The present
      degree seeks to broaden employment opportunities for
      our students in careers where mediated communication
      proliferates.
    </p>
        </div>
      
        <div className='col-md-6  col-sm-12 mb-2  rounded semester bg-white'>
        <h3 className='rounded'>Eligibility Criteria</h3>
    <hr />
    <ul>
      <li>Intermediate</li>
      <li>A level or Equivalent</li>
    </ul>
    <h4 className='paragraph rounded'>Available Seats </h4>
    <hr />
    Only 100 seats are Available for this Program.
        </div>
       
          </div>
          <div className='row mt-5'>
        <div className='col-md-6  col-sm-12 mb-2 semester  rounded bg-white '>
        <h2 className='rounded'>1st Semester</h2>
    <hr />
    <table className='table'>
      <tbody>
        <tr>
          <th>Course Code</th>
          <th>Course Tital</th>
          <th>Course ID</th>
        </tr>
        <tr>
          <td>ISE-111 </td>
          <td>Islamiat/Ethics </td>
          <td>2</td>
        </tr>
        <tr>
          <td>ENG-111 </td>
          <td>English I (Language in use) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>COMP-111 </td>
          <td>Computer (Intro & App) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>STAT-101 </td>
          <td>Statistic-I </td>
          <td>3</td>
        </tr>
        <tr>
          <td>BSCS-101 </td>
          <td>Introduction to Mass Communication </td>
          <td>1</td>
        </tr>
        <tr>
          <td>GEND-111 </td>
          <td>Gender Studies-I (MAS) </td>
          <td>3</td>
        </tr>{' '}
        <tr>
          <td>SOC-111 </td>
          <td>Sociology-I (MAS) </td>
          <td>3</td>
        </tr>
      </tbody>
    </table>
        
          </div>
          <div className='col-md-6  col-sm-12 mb-2 semester  rounded bg-white '>
          <h2 className='rounded'>2nd Semester</h2>
    <hr />
    <table className='table'>
      <tbody>
        <tr>
          <th>Course Code</th>
          <th>Course Tital</th>
          <th>Course ID</th>
        </tr>
        <tr>
          <td>PST-111 </td>
          <td>Pakistan Studies </td>
          <td>2</td>
        </tr>
        <tr>
          <td>ENG-112 </td>
          <td>English II (Acadenic R & W) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>BSCS-102 </td>
          <td>National and International Affairs </td>
          <td>3</td>
        </tr>
        <tr>
          <td>SOC-112 </td>
          <td>Sociology-II (MAS) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>GEND-112 </td>
          <td> Gender Studies-II (MAS) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-111 </td>
          <td>Elementary Mathematics-I (Algebra) </td>
          <td>3</td>
        </tr>
      </tbody>
    </table>
        
          </div>
        </div>
        <div className='row mt-5'>
        <div className='col-md-6  col-sm-12 mb-2  rounded semester bg-white'>
        <h2 className='rounded'>3rd Semester</h2>
    <hr />
    <table className='table'>
      <tbody>
        <tr>
          <th>Course Code</th>
          <th>Course Tital</th>
          <th>Course ID</th>
        </tr>
        <tr>
          <td>ENG-221 </td>
          <td>English III (Bus. Communication I) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>URDU-211 </td>
          <td>Functional Urdu </td>
          <td>3</td>
        </tr>
        <tr>
          <td>BSCS-201 </td>
          <td> National and International Affairs </td>
          <td>3</td>
        </tr>
        <tr>
          <td>POL-221 </td>
          <td>Political Science-I (MAS) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>APSY-211 </td>
          <td> Psychology-I (MAS) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>STAT-211 </td>
          <td>Elementary Stats </td>
          <td>3</td>
        </tr>
      </tbody>
    </table>
        
        </div>
        <div className='col-md-6  col-sm-12 mb-2  rounded semester bg-white'>
        <h2 className='rounded'>4th Semester</h2>
    <hr />
    <table className='table'>
      <tbody>
        <tr>
          <th>Course Code</th>
          <th>Course Tital</th>
          <th>Course ID</th>
        </tr>
        <tr>
          <td>ENG-212 </td>
          <td>English IV (English for Practical Aims) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>BSCS-202 </td>
          <td>Mass Media in Pakistan </td>
          <td>3</td>
        </tr>
        <tr>
          <td>ENG-223 </td>
          <td>Writing Workshop (MAS) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>POL-222 </td>
          <td>Political Science-II (MAS) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>APSY-212 </td>
          <td>Psychology-II (MAS) </td>
          <td>3</td>
        </tr>
      </tbody>
    </table>
      
        </div>

        </div>
        <div className='row mt-5'>
        <div className='col-md-6  col-sm-12 mb-2  rounded semester bg-white'>
        <h2 className='rounded'>5th Semester</h2>
    <hr />
    <table className='table'>
      <tbody>
        <tr>
          <th>Course Code</th>
          <th>Course Tital</th>
          <th>Course ID</th>
        </tr>
        <tr>
          <td>BSCS-301 </td>
          <td>News Reporting & Writing </td>
          <td>3</td>
        </tr>
        <tr>
          <td>BSCS-302 </td>
          <td>Broadcast Journalism </td>
          <td>3</td>
        </tr>
        <tr>
          <td>BSCS-303 </td>
          <td> Online Journalism </td>
          <td>3</td>
        </tr>
        <tr>
          <td>BSCS-304 </td>
          <td>Media Seminar </td>
          <td>3</td>
        </tr>
        <tr>
          <td>BSCS-305 </td>
          <td>Theories of Communication-I </td>
          <td>3</td>
        </tr>
      </tbody>
    </table>
      
        </div>
        <div className='col-md-6  col-sm-12 mb-2  rounded semester bg-white'>
        <h2 className='rounded'>6th Semester</h2>
    <hr />
    <table className='table'>
      <tbody>
        <tr>
          <th>Course Code</th>
          <th>Course Tital</th>
          <th>Course ID</th>
        </tr>
        <tr>
          <td>BSCS-306 </td>
          <td>
            Sub Editing & Page Designing (Theory &
            Practice){' '}
          </td>
          <td>3</td>
        </tr>
        <tr>
          <td>BSCS-307 </td>
          <td>
            Feature, Column & Editorial Writing (Theory &
            Practice){' '}
          </td>
          <td>3</td>
        </tr>
        <tr>
          <td>BSCS-308 </td>
          <td> Research Methods-I </td>
          <td>3</td>
        </tr>
        <tr>
          <td>BSCS-309 </td>
          <td>Theories of Communication-II </td>
          <td>3</td>
        </tr>
        <tr>
          <td>BSCS-310 </td>
          <td>
            {' '}
            Introduction to Advertising & Public Relations{' '}
          </td>
          <td>3</td>
        </tr>
      </tbody>
    </table>
        
        </div>
        </div>
    
      </div>
    </>
  );
}

export default BsMass_Communication;
