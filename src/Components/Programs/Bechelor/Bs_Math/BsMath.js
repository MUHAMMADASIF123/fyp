import React from 'react';
import './BsMath.css';

function BsMath() {
  return (
    <>
    <div className='container mt-2'>
      <div className='row'>
        <div className='col-md-6  col-sm-12 mb-2 semester  rounded bg-white '>
        <h2 className='rounded'>
      Bachelor of Science In Mathematics (Hons)
    </h2>
    <hr />
    <h4 className='paragraph rounded'>Course Object</h4>
    <hr />
    <p className='paragraph'>
      A Bachelor of Mathematics is an undergraduate
      academic degree awarded for successfully completing a
      program of study in mathematics or related
      disciplines, such as computer science or
      statistics.This program spreads over 8 semesters and
      covers 132 credit hours in 4 years.
    </p>
        </div>
      
        <div className='col-md-6  col-sm-12 mb-2  rounded semester bg-white'>
       
        <h3 className='rounded'>Eligibility Criteria</h3>
    <hr />
    <ul>
      <li>Intermediate</li>
      <li>A level or Equivalent</li>
    </ul>
    <h4 className='paragraph rounded'>Available Seats </h4>
    <hr />
    Only 100 seats are Available for this Program.
        </div>
       
          </div>
          <div className='row mt-5'>
        <div className='col-md-6  col-sm-12 mb-2 semester  rounded bg-white '>
        <h2 className='rounded'>1st Semester</h2>
    <hr />
    <table className='table'>
      <tbody>
        <tr>
          <th>Course Code</th>
          <th>Course Tital</th>
          <th>Course ID</th>
        </tr>
        <tr>
          <td>ISE-111 </td>
          <td>Islamiat/Ethics </td>
          <td>2</td>
        </tr>
        <tr>
          <td>ENG-111 </td>
          <td>English I (Language in use) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>CMP-111 </td>
          <td>Computer (Intro & App) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-101 </td>
          <td>Mathematics A-I [Calculus (I)] </td>
          <td>4</td>
        </tr>
        <tr>
          <td>MTH-102 </td>
          <td>
            Mathematics B-I [Vectors & Mechanics (I)]{' '}
          </td>
          <td>4</td>
        </tr>
        <tr>
          <td>PHY-111 </td>
          <td>Physics-I (Mechanics & Optics) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>PHY-112 </td>
          <td>Physics Lab-I (Mechanics & Optics) </td>
          <td>1</td>
        </tr>
      </tbody>
    </table>
        
          </div>
          <div className='col-md-6  col-sm-12 mb-2 semester  rounded bg-white '>
          <h2 className='rounded'>2nd Semester</h2>
    <hr />
    <table className='table'>
      <tbody>
        <tr>
          <th>Course Code</th>
          <th>Course Tital</th>
          <th>Course ID</th>
        </tr>
        <tr>
          <td>PST-111 </td>
          <td>Pakistan Studies </td>
          <td>2</td>
        </tr>
        <tr>
          <td>ENG-112 </td>
          <td>English II (Acadenic R & W) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-103 </td>
          <td>
            Mathematics A-II [Plane Curves & Analytic
            Geometry]{' '}
          </td>
          <td>4</td>
        </tr>
        <tr>
          <td>MATH-104 </td>
          <td>Mathematics B-II [Mechanics (II)] </td>
          <td>4</td>
        </tr>
        <tr>
          <td>PHY-113 </td>
          <td> Physics-II (Waves & Oscillation) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>PHY-114 </td>
          <td> Physics Lab-II (Waves & Oscillation) </td>
          <td>1</td>
        </tr>
        <tr>
          <td>MATH-105 </td>
          <td> Discrete Mathematics </td>
          <td>2</td>
        </tr>
      </tbody>
    </table>
        
          </div>
        </div>
        <div className='row mt-5'>
        <div className='col-md-6  col-sm-12 mb-2  rounded semester bg-white'>
        <h2 className='rounded'>3rd Semester</h2>
    <hr />
    <table className='table'>
      <tbody>
        <tr>
          <th>Course Code</th>
          <th>Course Tital</th>
          <th>Course ID</th>
        </tr>
        <tr>
          <td>ENG-221 </td>
          <td>English III (Bus. Communication I) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-201 </td>
          <td>Mathematics A-III [Linear Algebra] </td>
          <td>4</td>
        </tr>
        <tr>
          <td>MATH-202 </td>
          <td> Mathematics B-III [Calculus (II)] </td>
          <td>4</td>
        </tr>
        <tr>
          <td>PHY-211 </td>
          <td>Physics-III (Electricity & Magnetism) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>PHY-212 </td>
          <td>
            Physics Lab-III (Electricity & Magnetism){' '}
          </td>
          <td>1</td>
        </tr>
        <tr>
          <td>MATH-205 </td>
          <td> Graph Theory </td>
          <td>2</td>
        </tr>
      </tbody>
    </table>
        
        </div>
        <div className='col-md-6  col-sm-12 mb-2  rounded semester bg-white'>
        <h2 className='rounded'>4th Semester</h2>
    <hr />
    <table className='table'>
      <tbody>
        <tr>
          <th>Course Code</th>
          <th>Course Tital</th>
          <th>Course ID</th>
        </tr>
        <tr>
          <td>ENG-212 </td>
          <td>English-IV (English for Practical Aims) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-203 </td>
          <td>
            Mathematics A-IV [Ordinary Differential
            Equations]{' '}
          </td>
          <td>4</td>
        </tr>
        <tr>
          <td>MATH-204 </td>
          <td>
            Mathematics B-IV [Metric Spaces & Group Theory]{' '}
          </td>
          <td>4</td>
        </tr>
        <tr>
          <td>PHY-213 </td>
          <td>Physics-IV (Concepts of Modern Physics) </td>
          <td>3</td>
        </tr>
        <tr>
          <td>PHY-214 </td>
          <td>
            Physics Lab-IV (Concepts Modern Physics){' '}
          </td>
          <td>1</td>
        </tr>
        <tr>
          <td>MATH-206 </td>
          <td>Elementary Number Theory </td>
          <td>2</td>
        </tr>
        <tr>
          <td>SOC-211 </td>
          <td>Introduction to Sociology </td>
          <td>3</td>
        </tr>
      </tbody>
    </table>
      
        </div>

        </div>
        <div className='row mt-5'>
        <div className='col-md-6  col-sm-12 mb-2  rounded semester bg-white'>
        <h2 className='rounded'>5th Semester</h2>
    <hr />
    <table className='table'>
      <tbody>
        <tr>
          <th>Course Code</th>
          <th>Course Tital</th>
          <th>Course ID</th>
        </tr>
        <tr>
          <td>MATH-301 </td>
          <td>Real Analysis –I </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-302 </td>
          <td>Group Theory-I </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-303 </td>
          <td> Complex Analysis-I </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-304 </td>
          <td>Vector and Tensor Analysis </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-305 </td>
          <td> Topology </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-306 </td>
          <td> Differential Geometry </td>
          <td>3</td>
        </tr>
      </tbody>
    </table>
      
        </div>
        <div className='col-md-6  col-sm-12 mb-2  rounded semester bg-white'>
        <h2 className='rounded'>6th Semester</h2>
    <hr />
    <table className='table'>
      <tbody>
        <tr>
          <th>Course Code</th>
          <th>Course Tital</th>
          <th>Course ID</th>
        </tr>
        <tr>
          <td>MATH-307 </td>
          <td>Real Analysis –II </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-308 </td>
          <td>Rings and Vector Spaces </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-309 </td>
          <td> Complex Analysis – II </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-310 </td>
          <td>Mechanics </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-311 </td>
          <td>Functional Analysis-I </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-312 </td>
          <td> Ordinary Differential Equations </td>
          <td>3</td>
        </tr>
      </tbody>
    </table>
        
        </div>
        </div>
        <div className='row mt-5'>
        <div className='col-md-6  col-sm-12 mb-2  rounded semester bg-white'>
        <h2 className='rounded'>7th Semester</h2>
    <hr />
    <table className='table'>
      <tbody>
        <tr>
          <th>Course Code</th>
          <th>Course Tital</th>
          <th>Course ID</th>
        </tr>
        <tr>
          <td>MATH-401</td>
          <td>Set Theory</td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-402</td>
          <td>Partial Differential Equations</td>

          <td>3</td>
        </tr>
        <tr>
          <td>MATH-403</td>
          <td>Numerical Analysis-I </td>

          <td>3</td>
        </tr>
        <tr className='option_css'>
          <td>-</td>
          <td>Any two of the following</td>
          <td>-</td>
        </tr>
        <tr>
          <td>MATH-404 </td>
          <td>Mathematical Statistics-I </td>
          <td>3</td>
        </tr>{' '}
        <tr>
          <td>MATH-405 </td>
          <td>Fortran Programming</td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-406 </td>
          <td>Group Theory-II </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-407 </td>
          <td>Ring Theory </td>
          <td>3</td>
        </tr>{' '}
        <tr>
          <td>MATH-408</td>
          <td>Number Theory-I </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-409 </td>
          <td>Quantum Mechanics-I </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-410</td>
          <td>Analytical Dynamics</td>
          <td>3</td>
        </tr>{' '}
        <tr>
          <td>MATH-411 </td>
          <td>Electromagnetic Theory-I </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-412 </td>
          <td>Operations Research-I </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-413 </td>
          <td>Theory of Approximation and Splines-I</td>
          <td>3</td>
        </tr>{' '}
        <tr>
          <td>MATH-414</td>
          <td>Functional Analysis- II </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-415</td>
          <td>Fluid Mechanics-I </td>
          <td>3</td>
        </tr>
      </tbody>
    </table>
        
        </div>
        <div className='col-md-6  col-sm-12 mb-2  rounded semester bg-white'>
        <h2 className='rounded'>8th Semester</h2>
    <hr />
    <table className='table'>
      <tbody>
        <tr>
          <th>Course Code</th>
          <th>Course Tital</th>
          <th>Course ID</th>
        </tr>
        <tr>
          <td>MATH-416</td>
          <td>Measure Theory and Lebesgue Integration</td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-417</td>
          <td>Methods of Mathematical Physics</td>

          <td>3</td>
        </tr>
        <tr>
          <td>MATH-418</td>
          <td>Numerical Analysis-II </td>

          <td>3</td>
        </tr>
        <tr className='option_css'>
          <td>-</td>
          <td>Any two of the following</td>
          <td>-</td>
        </tr>
        <tr>
          <td>MATH-419 </td>
          <td>Mathematical Statistics-II </td>
          <td>3</td>
        </tr>{' '}
        <tr>
          <td>MATH-421 </td>
          <td>Computer Applications </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-422 </td>
          <td>Group Theory-III </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-423 </td>
          <td>Theory of Modules </td>
          <td>3</td>
        </tr>{' '}
        <tr>
          <td>MATH-424</td>
          <td>Number Theory-II </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-425 </td>
          <td>Quantum Mechanics-II </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-426</td>
          <td>Special theory of Relativity</td>
          <td>3</td>
        </tr>{' '}
        <tr>
          <td>MATH-427 </td>
          <td>Electromagnetic Theory-II </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-428 </td>
          <td>Operations Research-II </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-429 </td>
          <td>Theory of Approximation and Splines-II</td>
          <td>3</td>
        </tr>{' '}
        <tr>
          <td>MATH-430</td>
          <td>Functional Analysis-III </td>
          <td>3</td>
        </tr>
        <tr>
          <td>MATH-431</td>
          <td>Fluid Mechanics-II </td>
          <td>3</td>
        </tr>
      </tbody>
    </table>
        
        </div>
        </div>
      </div>
    </>
  );
}

export default BsMath;
